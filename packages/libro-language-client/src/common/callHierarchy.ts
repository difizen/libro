...
import type {
  ...
  CallHierarchyItem as VCallHierarchyItem,
  CallHierarchyIncomingCall as VCallHierarchyIncomingCall,
  CallHierarchyOutgoingCall as VCallHierarchyOutgoingCall,
  CancellationToken,
  CallHierarchyProvider as VCallHierarchyProvider,
} from 'vscode';
...
class CallHierarchyProvider implements VCallHierarchyProvider {
  ...
  public provideCallHierarchyIncomingCalls(
    ...
    const provideCallHierarchyIncomingCalls: CallHierarchyIncomingCallsSignature = (
      ...
      return client
        .sendRequest(CallHierarchyIncomingCallsRequest.type, params, token)
        .then(
          (result) => {
            if (token.isCancellationRequested) {
              return null;
  ...
  public provideCallHierarchyOutgoingCalls(
    ...
    const provideCallHierarchyOutgoingCalls: CallHierarchyOutgoingCallsSignature = (
      ...
      return client
        .sendRequest(CallHierarchyOutgoingCallsRequest.type, params, token)
        .then(
          (result) => {
            if (token.isCancellationRequested) {
              return null;
...
export class CallHierarchyFeature extends TextDocumentLanguageFeature<
...
> {
  ...
  public fillClientCapabilities(cap: ClientCapabilities): void {
    const capabilities: ClientCapabilities & CallHierarchyClientCapabilities =
