...
import type {
  ...
  CancellationToken,
  TypeHierarchyProvider as VTypeHierarchyProvider,
  TypeHierarchyItem as VTypeHierarchyItem,
} from 'vscode';
...
export type PrepareTypeHierarchySignature = (
  this: void,
...
export type TypeHierarchySupertypesSignature = (
  this: void,
  item: VTypeHierarchyItem,
  token: CancellationToken,
) => ProviderResult<VTypeHierarchyItem[]>;

export type TypeHierarchySubtypesSignature = (
  this: void,
  item: VTypeHierarchyItem,
  token: CancellationToken,
) => ProviderResult<VTypeHierarchyItem[]>;
...
class TypeHierarchyProvider implements VTypeHierarchyProvider {
  ...
  public prepareTypeHierarchy(
    ...
    const prepareTypeHierarchy: PrepareTypeHierarchySignature = (
      document,
      position,
      token,
    ) => {
  ...
  public provideTypeHierarchySupertypes(
    item: VTypeHierarchyItem,
    token: CancellationToken,
  ): ProviderResult<VTypeHierarchyItem[]> {
    const client = this.client;
    const middleware = this.middleware;
    const provideTypeHierarchySupertypes: TypeHierarchySupertypesSignature = (
      item,
      token,
    ) => {
      ...
      return client
        .sendRequest(TypeHierarchySupertypesRequest.type, params, token)
        .then(
          (result) => {
            if (token.isCancellationRequested) {
              return null;
  ...
  public provideTypeHierarchySubtypes(
    item: VTypeHierarchyItem,
    token: CancellationToken,
  ): ProviderResult<VTypeHierarchyItem[]> {
    const client = this.client;
    const middleware = this.middleware;
    const provideTypeHierarchySubtypes: TypeHierarchySubtypesSignature = (
      item,
      token,
    ) => {
...
export class TypeHierarchyFeature extends TextDocumentLanguageFeature<
...
> {
  ...
  protected registerLanguageProvider(
    options: TypeHierarchyRegistrationOptions,
  ): [Disposable, TypeHierarchyProvider] {
